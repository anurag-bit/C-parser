/*
 * STMicroelectronics ism303dac_core.c driver
 *
...
EXPORT_SYMBOL(st_ism303dac_sensor_write_data);
...
EXPORT_SYMBOL(st_ism303dac_sensor_input_init);
...
EXPORT_SYMBOL(st_ism303dac_sensor_input_cleanup);
...
EXPORT_SYMBOL(st_ism303dac_sensor_report_3axes_event);
...
void st_ism303dac_sensor_report_single_event(struct st_sensor_data *sdata, s32 data)
{
	...
	if (!sdata->enabled)
		return;
	st_ism303dac_sensor_input_init(sdata, sdata->name, false);
...
EXPORT_SYMBOL(st_ism303dac_sensor_report_single_event);
...
EXPORT_SYMBOL(st_ism303dac_sensor_poll_function);
...
EXPORT_SYMBOL(st_ism303dac_sensor_get_enable);
...
EXPORT_SYMBOL(st_ism303dac_sensor_set_enable);
...
EXPORT_SYMBOL(st_ism303dac_sensor_get_polling_rate);
...
EXPORT_SYMBOL(st_ism303dac_sensor_set_polling_rate);

#if defined(CONFIG_OF)
static u32 st_ism303dac_sensor_parse_dt(struct st_common_data *cdata)
{
	u32 val;
...
int st_ism303dac_sensor_common_probe(struct st_common_data *cdata, int irq)
{
	...
	if (irq > 0) {
#if defined(CONFIG_OF)
		err = st_ism303dac_sensor_parse_dt(cdata);
		if (err < 0)
			return err;
#else /* CONFIG_OF */
...
EXPORT_SYMBOL(st_ism303dac_sensor_common_probe);

void st_ism303dac_sensor_common_remove(struct st_common_data *cdata)
{
}
EXPORT_SYMBOL(st_ism303dac_sensor_common_remove);
...
EXPORT_SYMBOL(st_ism303dac_sensor_common_suspend);

int st_ism303dac_sensor_common_resume(struct st_common_data *cdata)
{
	return 0;
}
EXPORT_SYMBOL(st_ism303dac_sensor_common_resume);
